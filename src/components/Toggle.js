...
const Toggle = ({ label, onPowerChange }) => {
  ...
  return (
    <div className="inline-flex flex-col items-center text-lg uppercase underline m-4 p-4">
      ...
      <div
        onClick={() => onPowerChange(handlePowerChange)}
        className={
          isenabled
            ? "flex w-32 h-16 rounded-full items-center p-2 justify-end transition-colors duration-50 ease-in bg-green-600 cursor-pointer"
            : "flex w-32 h-16 rounded-full items-center p-2 justify-start transition-colors duration-50 ease-in bg-red-600 cursor-pointer"
        }
      >
